...
function removeFirstOccurrences(str, value) {
  ...
  const lastInd = startInd + value.length;
  const newStr = str.slice(0, startInd) + str.slice(lastInd, str.length);
  return newStr;
}
...
function removeLastOccurrences(str, value) {
  ...
  const startInd = str.lastIndexOf(value);
  const lastInd = startInd + value.length;
  const newStr = str.slice(0, startInd) + str.slice(lastInd, str.length);
  return newStr;
}
...
function sumOfCodes(str) {
  if (str) {
    ...
    return resArr.map((el) => el.charCodeAt()).reduce((acc, cur) => acc + cur);
  }
  return 0;
}
...
function orderAlphabetically(str) {
  let result;
  if (str.indexOf(str.search(/\d+/g)) !== -1) {
    const numbers = str.match(/\d+/g).sort((a, b) => a - b);
    const letters = str.match(/[a-zA-Z]+/g).sort((a, b) => a - b);
    result = numbers.join('') + letters.join('');
  } else {
...
function findLongestWord(sentence) {
  const arr = sentence.split(' ').sort((w1, w2) => w2.length - w1.length);
  return arr[0];
}
